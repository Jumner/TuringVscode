...
import { hover, moduleRegex, moduleSubRegex } from './hover/hoverCompletions';

export const moduleHoverProvider = vscode.languages.registerHoverProvider(
	't',
	{
		provideHover(doc : vscode.TextDocument, pos : vscode.Position) {
			...
			if (line.search(/(?<=Config.)(Display|Lang|Machine)/) >= 0) {
				...
				else if(sWord === 'Lang') return hover('Config.Lang returns information about the language and the limitations of the implementation that the program is currently running. The parameter langCode determines what sort of information is passed back', 'Config.Lang (langCode : int) : int', '(Function)', ['langCode'], ['Info to return. Either: clRelease, clLanguageVersion, clMaxNumStreams, clMaxNumDirStreams, clMaxNumRunTimeArgs'], 'Information about the Turing');
				else if(sWord === 'Machine') return hover('Config.Machine returns information about the machine that the program is currently running on. The parameter machineCode determines what sort of information is passed back', 'Config.Machine (machineCode : int) : int', '(Function)', ['machineCode'], ['Info to return. Either: cmProcessor, cmFPU, or cmOS'], 'Information on the current machine');
			}
			else if(line.search(/(?<=Dir.)(Open|GetLong|Get|Close|Create|Delete|Change|Current|Exists)/) >= 0) {
				if(sWord === 'Change') return hover('Dir.Change changes the execution directory to that specified by the parameter directoryPathName. This is the equivalent of doing a cd in UNIX', 'Dir.Change (directoryPathName : string)', '(Procedure)', ['directoryPathName'], ['Path to directory to change into']);
				else if(sWord === 'Close') return hover('Dir.Close is part of a series of four subprograms that help users get directory listings. Dir.Close is used to close a directory stream number opened by Dir.Open. After the directory stream number is closed, it can not be used with Dir.Get or Dir.GetLong', 'Dir.Close (streamNumber : int)', '(Procedure)', ['streamNumber'], ['File stream id']);
				else if(sWord === 'Create') return hover('Dir.Create is used to create the directory specified by the parameter directoryPathName. This is the equivalent of doing a mkdir in DOS or UNIX', 'Dir.Create (directoryPathName : string)', '(Procedure)', ['directoryPathName'], ['Path to directory to change into']);
				else if(sWord === 'Current') return hover('Dir.Current returns the full path name of the current execution directory. This is the equivalent of doing a pwd in UNIX', 'Dir.Current : string', '(Constant)', [], [], 'Current directory');
				else if(sWord === 'Delete') return hover('Dir.Delete is used to delete the directory specified by the parameter directoryPathName. This is the equivalent of doing a rmdir in DOS or UNIX. Dir.Delete will fail if it attempts delete a directory that has files in it', 'Dir.Delete (directoryPathName : string)', '(Procedure)', ['directoryPathName'], ['Path to directory to change into']);
				else if(sWord === 'Exists') return hover('Dir.Exists returns true if a directory by the name of directoryPathName exists. It will return false if directoryPathName is a file', 'Dir.Exists (directoryPathName : string) : boolean', '(Function)', ['directoryPathName'], ['Path to directory to change into'], 'If path exists');
				else if(sWord === 'Get') return hover('Dir.Get is part of a series of four subprograms that help users get directory listings. Dir.Get is used to get the file names in the directory. Each time the function is called, it returns the next file name in the directory', 'Dir.Get (streamNumber : int) : string', '(Function)', ['streamNumber'], ['File stream id'], 'A file in the current streamNumber directory');
				else if(sWord === 'GetLong') return hover('Dir.GetLong is part of a series of four subprograms that help users get directory listings. Dir.GetLong is used to get the names and assorted information of the files in the directory. Each time the function is called, it returns the name and information of the next file in the directory', 'Dir.GetLong (streamNumber : int, entryName : string, size, attribute, fileTime : int)', '(Procedure)', ['streamNumber', 'entryName', 'size', 'attribute', 'fileTime'], ['File stream id', 'Name of file', 'Size of file', 'File attributes', 'Last file modification']);
				else if(sWord === 'Open') return hover('Dir.Open is part of a series of four subprograms that help users get directory listings. Dir.Open returns a directory stream number if the directory could be opened', 'Dir.Open (directoryPathName : string) : int', '(Function)', ['directoryPathName'], ['Path to directory to change into'], 'Stream number id');
			}
				...
				else if(sWord === 'Trip') return hover('Error.Trip is a procedure that sets the error number that is returned by Error.Last and Error.LastMsg. It does not halt the program', 'Error.Trip (errorCode : int)', '(Procedure)', ['errorCode'], ['Error code']);
				else if(sWord === 'TripMsg') return hover('Error.TripMsg is a procedure that sets the error number and error message that is returned by Error.Last and Error.LastMsg. It does not halt the program', 'Error.TripMsg (errorCode : int, errorMessage : string)', '(Procedure)', ['errorCode', 'errorMessage'], ['Error Code','Error code message']);
				else if(sWord === 'Halt') return hover('Error.Halt is a procedure that immediately halts execution of the program and shows the specified error message on the line in the program that calls Error.Halt', 'Error.Halt (errorMessage : string)', '(Procedure)', ['errorMessage'], ['Error code message']);
			}
			else if(line.search(/(?<=File.)(Exists|FullPath|Parent|Status|Copy|Rename|Delete|DiskFree)/) >= 0) {
				if(sWord === 'Exists') return hover('File.Exists returns true if a file by the name of pathName exists. It will return false if pathName is a directory', 'File.Exists (pathName : string) : boolean', '(Function)', ['pathName'], ['Path to file'], 'If file at pathName exists');
				else if(sWord === 'FullPath') return hover('File.FullPath returns a string representing the full absolute path name in Turing format (forward slashes) of the path that is passed to the function. The path name passed in does not have to describe an existing file or directory', 'File.FullPath (pathName) : string) : string', '(Function)', ['pathName'], ['Path to file'], 'Absolute path to file');
				else if(sWord === 'Parent') return hover('File.Parent returns a string representing the parent directory in Turing format (forward slashes) of the path passed as a parameter. The path name passed in does not have to describe an existing file or directory', 'File.Parent (pathName : string) : string', '(Function)', ['pathName'], ['Path to file'], 'Path to parent directory');
				else if(sWord === 'Status') return hover('File.Status is used to get assorted information about a file or directory. When the function is called with a specified pathName, it returns the information about the file in the other parameters', 'File.Status (pathName : string, var size, attribute, fileTime : int)', '(Procedure)', ['pathName', 'size', 'attribute', 'fileTime'], ['Path to file', 'Variable parameter is set to size of file', 'Variable parameter is set to attributes of file', 'Time since last modification']);
				else if(sWord === 'Copy') return hover('File.Copy copies a file named by the srcPathName parameter to the file named by the destPathName parameter. The copy can be between different disks or file systems', 'File.Copy (srcPathName, destPathName : string)', '(Procedure)', ['srcPathName', 'destPathName'], ['Path to source file', 'Path to destination file']);
				else if(sWord === 'Rename') return hover('File.Copy renames a file or directory named by the srcPathName parameter to the destName parameter. The destName parameter must be a name only', 'File.Rename (srcPathName, destName : string)', '(Procedure)', ['srcPathName', 'destPathName'], ['Path to source file', 'Path to renamed file']);
				...
				else if(sWord === 'Radix') return hover('The "radix (usually 2)', 'Limits.Radix', '(Constant)');
				else if(sWord === 'NumDigits') return hover('The number of radix digits in f', 'Limits.NumDigits', '(Constant)');
				else if(sWord === 'MinExp') return hover('"minexp" (the smallest exponent allowed)', 'Limits.MinExp', '(Constant)');
				else if(sWord === 'MaxExp') return hover('"maxexp" (the largest exponent allowed)', 'Limits.MaxExp', '(Constant)');
				else if(sWord === 'Rreb') return hover('The relative round-off error bound', 'Limits.Rreb', '(Constant)');
			}
			else if(line.search(/(?<=Input.)(Pause|KeyDown|Flush)/) >= 0) {
				if(sWord === 'Pause') return hover('The Input.Pause procedure simply waits for a key to be pressed and then returns. It echoes the key pressed if echo mode is set. (See View.Set for setting echo mode)', 'Input.Pause', '(Procedure)');
				else if(sWord === 'KeyDown') return hover('The Input.Keydown procedure allows a program to read which keys are currently being pressed. This procedure is different from getch in that it allows a program to detect when a user presses and releases a button. As such, it is not to be used in conjunction with getch', 'Input.KeyDown (var chars : array char of boolean)', '(Procedure)', ['chars'], ['Variable chars of type array char of boolean']);
				else if(sWord === 'Flush') return hover('The Input.Flush procedure empties the keyboard buffer. It is often used to avoid accidentally reading multiple keystrokes because the user pressed a key for too long, causing autorepeat', 'Input.Flush', '(Procedure)');
			}
			else if(line.search(/(?<=Math.)(DistancePointLine|Distance|PI|E)/) >= 0) {
				if(sWord === 'PI') return hover('The constant Pi π = ~3.1415', 'Math.PI', '(Constant)');
				else if(sWord === 'E') return hover('Eulers number e = ~2.7183', 'Math.E', '(Constant)');
				else if(sWord === 'Distance') return hover('Math.Distance is used to calculate the distance between two points. (x1, y1) is the location of the first point, and (x2, y2) is the location of the second point', 'Math.Distance (x1, y1, x2, y2 : real) : real', '(Function)', ['x1', 'y1', 'x2', 'y2'], ['Point 1 X position', 'Point 1 Y position', 'Point 2 X position', 'Point 2 Y position'], 'Distance between input points');
				else if(sWord === 'DistancePointLine') return hover('Math.DistancePointLine is used to calculate the distance between a point and a line segment. It is often used in games to determine if a collision has occurred. (xp, yp) is the location between the point. (x1, y1) and (x2, y2) are the end points of the line segment', 'Math.DistancePointLine (xp, yp, x1, y1, x2, y2 : real) : real', '(Function)', ['xp', 'yp', 'x1', 'y1', 'x2', 'y2'], ['Point X position', 'Point Y position', 'Line point 1 X position', 'Line point 1 Y position', 'Line point 2 X position', 'Line point 2 Y position'], 'Min distance between input point and input line');
			}
			else if(line.search(/(?<=Mouse.)(Where|ButtonMoved|ButtonWait|ButtonChoose)/) >= 0) {
				if(sWord === 'Where') return hover('The Mouse.Where procedure is used to get current information about the status of the mouse. The parameters x and y are set to the current location of the mouse cursor. If the program is running on a system using windows, the cursor may be outside the window. This means that x and y may be set to values outside of the bounds of 0 to maxx and 0 to maxy', 'Mouse.Where (var x, y, button : int)', '(Procedure)', ['x', 'y', 'button'], ['Input variable set to mouse X position', 'Input variable set to mouse Y position', 'Input variable set to mouse button']);
				else if(sWord === 'ButtonMoved') return hover('The Mouse.ButtonMoved function indicates whether there is a mouse event of the appropriate type on the mouse queue. Events are either "up", "down", "updown" or "downup" events (although the "downup" and "updown" are the same event)', 'Mouse.ButtonMoved (motion : string) : boolean', '(Function)', ['motion'], ['Mouse event to ask for'], 'If mouse event occurred');
				else if(sWord === 'ButtonWait') return hover('The Mouse.ButtonWait procedure gets information about a mouse event and removes it from the queue', 'Mouse.ButtonWait (motion : string, var x, y, buttonNumber, buttonUpDown : int)', '(Procedure)', ['motion', 'x', 'y', 'buttonNumber', 'buttonUpDown'], ['Mouse event', 'Variable', 'Input variable set to mouse X position', 'Input variable set to mouse Y position', 'Input variable set to mouse number', 'Input variable set to button event']);
				else if(sWord === 'ButtonChoose') return hover('The Mouse.ButtonChoose procedure is used to change the mode of the mouse. In Turing, the mouse can either be in "single-button mode" or in "multi-button mode". In "single-button mode" the mouse is treated as a one button mouse. A button is considered pressed when any button is pressed and released only when all buttons have been released', 'Mouse.ButtonChoose (choice : string)', '(Procedure)', ['choice'], ['Mode to set mouse to']);
			}
			else if(line.search(/(?<=Music.)(PlayFileStop|PlayFileLoop|PlayFileReturn|PlayFile|Play|SoundOff|Sound)/) >= 0) {
				if(sWord === 'PlayFileStop') return hover('The Music.PlayFileStop procedure is used to to stop all music files currently playing. This includes processes that are executing the Music.PlayFile procedure (they exit immediately and start executing the next statement in the process), and the Music.PlayFileReturn and Music.PlayFileLoop statements, which simply stop playing the music', 'Music.PlayFileStop', '(Procedure)');
				else if(sWord === 'PlayFileLoop') return hover('The Music.PlayFileLoop procedure is used to play a file of music continuously, looping until the program is halted or the Music.PlayFileStop command is given. The file must be in one of the acceptable formats and the machine, must have the appropriate hardware', 'Music.PlayFileLoop ( fileName : string )', '(Procedure)', ['fileName'], ['Name of music file to play']);
				else if(sWord === 'PlayFileReturn') return hover('The Music.PlayFileReturn procedure is used to play a file of music. The file must be in one of the acceptable formats and the machine, must have the appropriate hardware', 'Music.PlayFileReturn ( fileName : string )', '(Procedure)', ['fileName'], ['Name of music file to play']);
				else if(sWord === 'PlayFile') return hover('The Music.PlayFile procedure is used to play a file of music. The file must be in one of the acceptable formats and the machine, must have the appropriate hardware', 'Music.PlayFile ( fileName : string )', '(Procedure)', ['fileName'], ['Name of music file to play']);
				else if(sWord === 'Play') return hover('The Music.Play procedure is used to sound musical notes on the computer', 'Music.Play ( music : string )', '(Procedure)', ['music'], ['String of notes to play']);
				else if(sWord === 'SoundOff') return hover('The Music.SoundOff procedure stops any sound or music that is currently playing or is waiting to play', 'Music.SoundOff', '(Procedure)');
				else if(sWord === 'Sound') return hover('The Music.Sound statement is used to cause the computer to sound a note of a given frequency for a given time. The frequency is in cycles per second (Hertz). The time duration is in milliseconds. For example, middle A on a piano is 440 Hertz, so Music.Sound(440, 1000) plays middle A for one second', 'Music.Sound ( frequency, duration : int )', '(Procedure)', ['frequency', 'duration'], ['Frequency of sound to play', 'Duration to play frequency']);
			}
				...
				if(sWord === 'WaitForConnection') return hover('Listens for a connection at the port specified by the port parameter. When another program connects to the port, then the function returns. The address of the connecting machine is specified in the netAddr parameter and the Net.WaitForConnection returns a network stream descriptor which can be used with the put, get, read, and write statements and eof function to send and receive data to the connecting program. It is also the parameter used for the Net.CloseConnection, Net.BytesAvailable, Net.CharAvailable, Net.LineAvailable, and Net.TokenAvailable functions', 'Net.WaitForConnection (port : int, var netAddr : string) : int', '(Function)', ['port', 'netAddr'], ['Port to listen on', 'Variable set to the address of connecting device'], 'Stream id to listen to data');
				else if(sWord === 'OpenConnection') return hover('Attempts to open a connection to port specified by the port parameter on the machine specified by netAddr parameter. There must be a program listening to that port for the connection to be made. In OOT, this is done using the Net.WaitForConnection function', 'Net.OpenConnection (netAddr : string, port : int) : int', '(Function)', ['netAddr', 'port'], ['Address to computer to send information to', 'Port to send information to'], 'Stream id to send data to');
				else if(sWord === 'OpenURLConnection') return hover('Attempts to open a http connection to pthe URL (Universal Resource Locator) specified by the urlAddr', 'Net.OpenURLConnection (urlAddr : string) : int', '(Function)', ['urlAddr'], ['Url to open a connection to'], 'Stream id to send data to');
				else if(sWord === 'CloseConnection') return hover('Closes a network connection made with Net.OpenConnection or Net.WaitForConnection. After the connection is closed, the net stream cannot be used for any purpose on either side of the connection', 'Net.CloseConnection (netStream : int)', '(Procedure)', ['netStream'], ['Id of net stream returned when connection was opened']);
				else if(sWord === 'BytesAvailable') return hover('Returns the number of bytes available for reading from the net stream specified by the netStream parameter', 'Net.BytesAvailable (netStream : int) : int', '(Function)', ['netStream'], ['Id of net stream returned when connection was opened'], 'Number of bytes of stream left to read');
				else if(sWord === 'CharAvailable') return hover('Returns true if a character is waiting to be read from the net stream specified by the netStream parameter. If Net.CharAvailable returns true, then a single character can be read from the stream without blocking', 'Net.CharAvailable (netStream : int) : boolean', '(Function)', ['netStream'], ['Id of net stream returned when connection was opened'], 'If a char is able to be read from stream');
				else if(sWord === 'LineAvailable') return hover('Returns true if a line of input is waiting to be read from the net stream specified by the netStream parameter. If Net.LineAvailable returns true, then a line of input can be read from the stream without blocking', 'Net.LineAvailable (netStream : int) : boolean', '(Function)', ['netStream'], ['Id of net stream returned when connection was opened'], 'If a line is able to be read from stream');
				else if(sWord === 'TokenAvailable') return hover('Returns true if a line of input is waiting to be read from the net stream specified by the netStream parameter. If Net.TokenAvailable returns true, then a single token (character surrounded by whitespace) can be read from the stream without blocking', 'Net.TokenAvailable (netStream : int) : boolean', '(Function)', ['netStream'], ['Id of net stream returned when connection was opened'], 'If a token is able to be read from stream');
				else if(sWord === 'HostAddressFromName') return hover('Returns the numeric TCP/IP address of the machine whose hostname is specified by the hostName parameter', 'Net.HostAddressFromName (hostName : string) : string', '(Function)', ['hostName'], ['Hostname of a machine'], 'Tcp/Ip address of hostname');
				else if(sWord === 'HostNameFromAddress') return hover('Returns the TCP/IP hostname of the machine whose numeric address is specified by the hostAddr parameter', 'Net.HostNameFromAddress (hostAddr : string) : string', '(Function)', ['hostAddr'], ['Tcp/Ip address of a machine'], 'Hostname of Tcp/Ip address');
				else if(sWord === 'LocalAddress') return hover('Returns the TCP/IP numeric address of the machine the program is running on. The numeric address is of the form xxx.yyy.zzz.www where each segment is a number from 0 to 255', 'Net.LocalAddress : string', '(Constant)', [], [], 'Local ip address of machine');
				else if(sWord === 'LocalName') return hover('Returns the TCP/IP hostname of the machine the program is running on', 'Net.LocalName : string', '(Constant)', [], [], 'Hostname of machine');
			}
			else if(line.search(/(?<=PC.)(ParallelGet|ParallelPut)/) >= 0) {
				if(sWord === 'ParallelGet') return hover('The PC.ParallelGet procedure is used on a PC to read the value of certain pins on the parallel port. This port corresponds to the MS-DOS device "LPT1". This procedure can be used to control robots and peripherals', 'PC.ParallelGet (port : int) : nat1', '(Function)', ['port'], ['Parallel port to receive data from'], 'Decimal of byte representing which pins are high. If high is = 1 and low = 0, Then number `88` represents `1011000` means Pin 11 is high, Pin 10 is low, Pin 12 is gigh, Pin 13 is high, Pin 15 is low');
				else if(sWord === 'ParallelPut') return hover('The PC.ParallelPut procedure is used on a PC to set the values on the data pins on the parallel port. This port corresponds to the MS-DOS device "LPT1". This procedure can be used to control robots and peripherals', 'PC.ParallelPut (port : int, value : int)', '(Function)', ['port', 'value'], ['Parallel port to send data do', 'Byte where each pin represents each place in the binary representation of the byte']);
			}
			else if(line.search(/(?<=Rand.)(Real|Int|Reset|Set|Next|Seed)/) >= 0) {
				if(sWord === 'Real') return hover('The Rand.Real function returns a pseudo-random number in the range zero to one. For example, if x is a real number, after x := Rand.Real, x would have a value such as 0.729548 or 0.352879', 'Rand.Real : real', '(Function)', [], [], 'A random number of type `real`');
				else if(sWord === 'Int') return hover('The Rand.Int statement is used to create a pseudo-random integer in the range low to high, inclusive. For example, if i is an integer, after randint(i,1, 10), i would have a value such as 7 or 2 or 10', 'Rand.Int (low, high : int) : int', '(Function)', ['low', 'high'], ['Lower bound of possible numbers', 'Upper bound of possible numbers'], 'A random number of type `int` between `low` and `high`');
				else if(sWord === 'Reset') return hover('This is a procedure with no parameters that resets the sequences of pseudo-random numbers produced by Rand.Real and Rand.Int. This allows identical executions of the same program to produce identical results', 'Rand.Reset', '(Procedure)');
				else if(sWord === 'Set') return hover('This procedure sets the seed for sequences of pseudo-random numbers produced by Rand.Real and Rand.Int. This allows identical executions of the same program to produce identical results', 'Rand.Set (seed : nat4)', '(Procedure)', ['seed'], ['Seed to set random to']);
				else if(sWord === 'Next') return hover('The Rand.Next procedure is used when you need several sequences of pseudo-random numbers, and you need to be able to exactly repeat these sequences for a number of simulations. The Rand.Next procedure is the same as rand, except seq specifies one of ten independent and repeatable sequences of pseudo-random real numbers', 'Rand.Next (seq : 1 .. 10) : real', '(Function)', ['seq'], ['Input sequence'], 'Output number of type real');
				else if(sWord === 'Seed') return hover('The Rand.Seed procedure restarts one of the sequences generated by Rand.Next. Each restart with the same seed causes Rand.Next to produce the same sequence for the given sequence', 'Rand.Seed (seed : nat4, seq : 1 .. 10)', '(Procedure)', ['seed', 'seq'], ['Seed to start sequence at', 'Input sequence']);
			}
			else if(line.search(/(?<=RGB.)(GetColour|SetColour|AddColour)/) >= 0) {
				if(sWord === 'GetColour') return hover('The RGB.GetColour procedure returns the red, green and blue components to the color associated with the colorNumber parameter. The red, green and blue values are normalized to be between 0 and 1. Thus color white returns 1.0 for the redComp, greenComp and blueComp values and colour black returns 0.0 for all three', 'RGB.GetColour(colourNumber : int, var redComp, greenComp, blueComp : real)', '(Procedure)', ['colourNumber', 'redComp', 'greenComp', 'blueComp'], ['Colour id to get', 'Variable to set red value to', 'Variable to set green value to', 'Variable to set blue value to']);
				else if(sWord === 'SetColour') return hover('The RGB.SetColour function sets the red, green and blue components of the color associated with the colourNumber parameter. The red, green and blue values must normalized to be between 0 and 1', 'RGB.SetColour(colourNumber : int, redComp, greenComp, blueComp : real)', '(Procedure)', ['colourNumber', 'redComp', 'greenComp', 'blueComp'], ['Colour id to set', 'Red value (0-1)', 'Green value (0-1)', 'Blue value (0-1)']);
				else if(sWord === 'AddColour') return hover('The RGB.AddColour function attempts to create a new colour with the red, green and blue components specified. If successful, the function returns a new color number (usually one greater than maxcolor) and maxcolor is updated by adding 1 to it. If it is unsuccessful, the function returns 1 and Error.Last and Error.LastMsg can be used to determine the cause of the problem', 'RGB.AddColour(redComp, blueComp, greenComp)', '(Function)', ['redComp', 'blueComp', 'greenComp'], ['Colour id to set', 'Red value (0-1)', 'Green value (0-1)', 'Blue value (0-1)'], 'Id of newly created colour');
			}
			else if(line.search(/(?<=Stream.)(FlushAll|Flush)/) >= 0) {
				if(sWord === 'Flush') return hover('The Stream.Flush procedure is used to flush any buffered output associated with the streamNumber parameter', 'Stream.Flush ( streamNumber : int )', '(Procedure)', ['streamNumber'], ['Id of stream to flush']);
				else if(sWord === 'FlushAll') return hover('The Stream.FlushAll procedure is used to flush any buffered output in any open file', 'Stream.FlushAll', '(Procedure)');
			}
			else if(line.search(/(?<=Str.)(Lower|Upper|Trim)/) >= 0) {
				if(sWord === 'Lower') return hover('The Str.Lower function takes the string s and returns a string in which all the upper case letters are converted to lower case. For example, Str.Lower ("ABC123def") returns "abc123def"', 'Str.Lower (s : string) : string', '(Function)', ['s'], ['Input string to put into lowercase'], 'Lowercase output string');
				else if(sWord === 'Upper') return hover('The Str.Upper function takes the string s and returns a string in which all the lower case letters are converted to upper case. For example, Str.Upper ("ABC123def") returns "ABC123DEF"', 'Str.Upper (s : string) : string', '(Function)', ['s'], ['Input string to put into uppercase'], 'Uppercase output string');
				else if(sWord === 'Trim') return hover('The Str.Trim function takes the string str and returns a string in all the leading and trailing spaces (the spaces at the beginning and the end) are deleted. For example, Str.Trim (" This is a test ") returns "This is a test". If str only has spaces in it, then Str.Trim will return an empty string', 'Str.Trim (str : string) : string', '(Function)', ['str'], ['Input string to trim'], 'Output trimmed string');
			}
			else if(line.search(/(?<=Sys.)(GetComputerName|GetEnv|GetPid|GetUserName|Exec|Nargs|FetchArg)/) >= 0) {
				if(sWord === 'GetComputerName') return hover('The Sys.GetComputerName function is used to determine the name of the computer. On the PC, this is the NetBIOS name. It returns “No Name” if a name could not be determined', ' 	Sys.GetComputerName : string', '(Function)', [], [], 'Computer name');
				else if(sWord === 'GetEnv') return hover('The Sys.GetEnv function is used to access the environment string whose name is symbol. These strings are determined by the shell (command processor) or the program that caused your program to run. See also the Sys.Nargs and Sys.FetchArg functions', 'Sys.GetEnv ( symbol : string ) : string', '(Function)', ['symbol'], ['Setting to get'], 'Resulting setting');
				else if(sWord === 'GetPid') return hover('The Sys.GetPid function is used to determine the I.D. (number) that identifies the current operating system task (process). Beware that there are processes, activated by the fork statement, that are independent of the operating systems tasks', 'Sys.GetPid : int', '(Function)', [], [], 'Pid of program');
				else if(sWord === 'GetUserName') return hover('The Sys.GetUserName function is used to determine the name of the current user. It returns “Unknown” if a name could not be determined', 'Sys.GetUserName : string', '(Function)', [], [], 'Username of computer');
				else if(sWord === 'Exec') return hover('The Sys.Exec function is used to execute an application or more often, open a data file with its associated application. Sys.Exec can be used to launch such programs as the Internet Browser by specifying a URL. Sys.Exec launches the application associated with file\'s suffix. (In essence, it performs the same operation as if a user double clicked on the file.)', 'Sys.Exec ( command : string ) : boolean', '(Function)', ['command'], ['System command to be run'], 'If the command executed properly');
				else if(sWord === 'Nargs') return hover('The Sys.Nargs function is used to determine the number of arguments that have been passed to a program from the command line. For example, if the program is run from the Turing environment using', 'Sys.Nargs : int', '(Constant)', [], [], 'Number of arguments passed in from the command line');
				else if(sWord === 'FetchArg') return hover('The Sys.FetchArg function is used to access the i-th argument that has been passed to a program from the command line', 'System.FetchArg ( i : int ) : string', '(Function)', ['i'], ['Which argument to access'], 'Output string of argument');
			}
			else if(line.search(/(?<=Text.)(Cls|ColourBack|Colour|LocateXY|Locate|WhatRow|WhatColourBack|WhatColour|WhatCol)/) >= 0) {
				if(sWord === 'Cls') return hover('The Text.Cls (clear screen) procedure is used to blank the screen to the text background color. The cursor is set to the top left (to row 1, column 1)', 'Text.Cls');
				else if(sWord === 'ColourBack') return hover('The Text.ColourBack procedure is used to change the current text background colour', 'Text.ColorBack ( Colour : int )', '(Procedure)', ['Colour'], ['Colour to set the background colour to']);
				else if(sWord === 'Colour') return hover('The Text.Colour procedure is used to change the currently-active colour. This is the colour of characters that are to be put on the screen', 'Text.Colour ( Colour : int )', '(Procedure)', ['Colour'], ['Colour to set the text to']);
				else if(sWord === 'LocateXY') return hover('The Text.LocateXY procedure is used to move the cursor so that the next output from put will be at approximately (x, y). The exact location may be somewhat to the left of x and below y to force alignment to a character boundary', 'Text.LocateXY ( x , y : int )', '(Procedure)', ['x', 'y'], ['X position to set curser to', 'Y position to set curser to']);
				else if(sWord === 'Locate') return hover('The Text.Locate procedure is used to move the cursor so that the next output from put will be at the given row and column. Row 1 is the top of the screen and column 1 is the left side of the screen', 'Text.Locate ( row, column : int )', '(Procedure)', ['row', 'column'], ['Row to set curser to', 'Column to set curser to']);
				else if(sWord === 'WhatRow') return hover('The Text.WhatRow function is used to determine the cursor position\'s row', 'Text.WhatRow : int', '(Constant)', [], [], 'Current row of curser');
				else if(sWord === 'WhatColourBack') return hover('The Text.WhatColourBack function is used to determine the current text background colour', ' 	Text.WhatColourBack : int', '(Constant)', [], [], 'Current background colour at the curser');
				else if(sWord === 'WhatColour') return hover('The Text.WhatColour function is used to determine the current text (foreground) colour, ie., the color used for characters that are output using put', 'Text.WhatColour : int', '(Constant)', [], [], 'Current colour at the curser');
				else if(sWord === 'WhatCol') return hover('The Text.WhatCol function is used to determine the cursor position\'s column', 'Text.WhatCol : int', '(Constant)', [], [], 'Current column of curser');
			}
			else if(line.search(/(?<=Time.)(SecDate|DateSec|SecParts|PartsSec|SecStr|DelaySinceLast|Delay|Sec|Date|ElapsedCPU|Elapsed)/) >= 0) {
				if(sWord === 'SecDate') return hover('The Time.SecDate function is used to convert the number of seconds since 00:00:00 GMT Jan 1, 1970 into a date and time string', 'Time.SecDate (timeInSecs : int) : string', '(Function)', ['timeInSecs'], ['Input time in seconds'], 'Output date as type string');
				else if(sWord === 'DateSec') return hover('The Time.DateSec function is used to convert a date and time string into a number, specifically, the number of seconds since 00:00:00 GMT Jan 1, 1970', 'Time.DateSec (dateString : string) : int', '(Function)', ['dateString'], ['Input date as string "dd mmm yy"'], 'Output time as seconds since Jan 1, 1970');
				else if(sWord === 'SecParts') return hover('The Time.SecParts function is used to convert a single number form of the time (the number of seconds since 00:00:00 GMT Jan 1, 1970) into a date with numeric component parts', ' 	Time.SecParts (sec : int, var year, month, day, dayOfWeek, hour, minute, second : int)', '(Procedure)', ['sec', 'year', 'month', 'day', 'dayOfWeek', 'hour', 'minute', 'second'], ['Seconds since Jan 1, 1970', 'Variable output year', 'Variable output month', 'Variable output day', 'Variable output dayOfWeek', 'Variable otuput hour', 'Variable output minute', 'Variable output second']);
				else if(sWord === 'PartsSec') return hover('The Time.PartsSec function is used to convert the numeric parts of a date (specifically the year, month, day, hour, minute and second) into the number of seconds since 00:00:00 GMT Jan 1, 1970 and the date specified by the parts', 'Time.PartsSec (year, month, day, hour, minute, second : int) : int', '(Function)', ['year', 'month', 'day', 'hour', 'minute', 'second'], ['Input year', 'Input month', 'Input day', 'Input hour', 'Input minute', 'Input second'], 'Output time as seconds since Jan 1, 1970');
				else if(sWord === 'SecStr') return hover('The Time.SecStr function is used to convert the number of seconds since 00:00:00 GMT Jan 1, 1970 into a date and time string', 'Time.SecStr (timeInSecs : int, formatString : string) : string', '(Function)', ['timeInSecs', 'formatString'], ['Seconds since Jan 1, 1970', 'Time [Format string](http://compsci.ca/holtsoft/doc/time_secstr.html)'], 'Formatted date string');
				else if(sWord === 'Delay') return hover('The Time.Delay procedure is used to cause the program to pause for a given time. The time duration is in milliseconds', 'Time.Delay ( duration : int )', '(Procedure)', ['duration'], ['Time to wait (ms)']);
				else if(sWord === 'DelaySinceLast') return hover('The Time.DelaySinceLast procedure is used to cause the program to pause for a given time since the last call to Time.DelaySinceLast. The time duration is in milliseconds', 'Time.DelaySinceLast ( duration : int )', '(Procedure)', ['duration'], ['Wait until `Duration` has passed since the start of execution']);
				else if(sWord === 'Sec') return hover('The Time.Sec function returns the current date and time as a number. The returned integer is the time in seconds since 00:00:00 GMT (Greenwich Mean Time) January 1, 1970', 'Time.Sec : int', '(Constant)', [], [], 'Seconds since Jan 1, 1970');
				else if(sWord === 'Date') return hover('The Time.Date function returns the current date and time as a string. The returned string in the format "dd mmm yy hh:mm:ss", where mmm is the first 3 characters of the month, e.g., "Apr". For example, if the date is Christmas 1989 at 9:02:37 in the morning, Time.Date will return "25 Dec 89 09:02:37". Twenty-four hour time is used, so eleven thirty at night the same day would return "25 Dec 89 23:30:00"', 'Time.Date : string', '(Constant)', [], [], 'Get date as string');
				else if(sWord === 'Elapsed') return hover('The Time.Elapsed function returns the amount of time since a program (process) started running. The number of milliseconds since the program started running is returned', 'Time.Elapsed : int', '(Constant)', [], [], 'Time since program start (ms)');
				else if(sWord === 'ElapsedCPU') return hover('The Time.ElapsedCPU function is used on a multitasking system such as UNIX to determine the amount of time that has been used by this program (process). The number of central processor milliseconds assigned to this program is returned. This is of little use on a personal computer, where Time.ElapsedCPU returns the same value as Time.Elapsed', 'Time.ElapsedCPU : int', '(Constant)', [], [], 'Time of CPU usage (ms)');
			}
			else if(line.search(/(?<=View.)(Set|ClipSet|ClipAddOval|ClipAdd|WhatDotColour|UpdateArea|Update|maxx|maxy|maxcolour)/) >= 0) {
				if(sWord === 'maxx') return hover('The View.maxx function is used to determine the maximum value of x for the current graphics mode', 'maxx : int', '(Constant)', [], [], 'Max x size');
				else if(sWord === 'maxy') return hover('The View.maxy function is used to determine the maximum value of y for the current graphics mode', 'maxy : int', '(Constant)', [], [], 'Max y size');
				else if(sWord === 'maxcolour') return hover('The View.maxcolour function is used to determine the maximum colour number for the current mode of the screen', 'View.maxcolour : int', '(Constant)', [], [], 'Max colour id');
				else if(sWord === 'Set') return hover('The View.Set statement is used to change the mode of the screen, as well as the way in which Turing does input and output. The parameter to View.Set is a string, such as "graphics". The string contains one or more options separated by commas, such as "text, noecho". View.Set affects the active window', 'View.Set ( s : string )', '(Procedure)', ['s'], ['Mode to set view to']);
				else if(sWord === 'ClipSet') return hover('The View.ClipSet procedure sets the clipping region to the rectangle specified by (x1, y1) - (x2, y2). If a clipping region already exist, it is replaced by the specified rectangle', 'View.ClipSet (x1, y1, x2, y2 : int)', '(Procedure)', ['x1', 'y1', 'x2', 'y2'], ['Corner 1 X position', 'Corner 1 Y position', 'Corner 2 X position', 'Corner 2 Y position']);
				else if(sWord === 'ClipAdd') return hover('The View.ClipAdd procedure adds another rectangle specified by (x1, y1) - (x2, y2) to the clipping region. This only works on systems that support complex clipping regions. If no clipping region has been specified, then the rectangle becomes the complete clipping region', 'View.ClipAdd (x1, y1, x2, y2 : int)', '(Procedure)', ['x1', 'y1', 'x2', 'y2'], ['Corner 1 X position', 'Corner 1 Y position', 'Corner 2 X position', 'Corner 2 Y position']);
				else if(sWord === 'ClipAddOval') return hover('The View.ClipAddOval procedure adds another oval specified by (x, y) and xradius and yradius) to the clipping region. If no clipping region has been specified, then the oval becomes the complete clipping region', ' 	View.ClipAddOval (x, y, xradius, yradius : int)', '(Procedure)', ['x', 'y', 'xradius', 'yradius'], ['Center X position', 'Center Y position', 'Radius along X axis', 'Radius along Y axis']);
				else if(sWord === 'ClipOff') return hover('The View.ClipOff procedure turns off clipping. This means that any drawing commands can appear on the entire drawing surface (the screen or the window, depending on the system)', 'View.ClipOff', '(Procedure)');
				else if(sWord === 'WhatDotColour') return hover('The View.WhatDotColour function is used to determine the colour number of the specified pixel', 'View.WhatDotColour ( x, y : int ) : int', '(Function)', ['x', 'y'], ['X position', 'y Position'], 'Output colour id');
				else if(sWord === 'Update') return hover('The View.Update procedure updates a Run window from an offscreen bitmap. It is used with the command View.Set ("offscreenonly") which prevents the Run window from being updated until the View.Update command is given', 'View.Update', '(Procedure)');
				else if(sWord === 'UpdateArea') return hover('The View.UpdateArea procedure updates a rectanglular area Run window, specified by (x1, y1) - (x2, y2) from the offscreen bitmap. It is used with the command View.Set ("offscreenonly") which prevents the Run window from being updated until the View.UpdateArea or View.Update command is given', 'View.UpdateArea (x1, y1, x2, y2)', '(Procedure)', ['x1', 'y1', 'x2', 'y2'], ['Corner 1 X position', 'Corner 1 Y position', 'Corner 2 X position', 'Corner 2 Y position']);
			}
				...
				if(sWord === 'Open') return hover('The Window.Open function is used to create a window. A window ID is returned if the window is successfully created. If the window is not created then it returns 0. Error.Last and Error.LastMsg can then be used to determine the cause of the failure', 'Window.Open (setUpString : string) : int', '(Function)', ['setUpString'], ['Properties of newly created window'], 'Window id');
				else if(sWord === 'Close') return hover('The Window.Close procedure closes the window specified by the windowID parameter', 'Window.Close (windowID : int)', '(Procedure)', ['windowID'], ['Window identifier to close']);
				else if(sWord === 'Select') return hover('The Window.Select selects the window that output is to be sent to', 'Window.Select (windowID : int)', '(Procedure)', ['windowID'], ['Window identifier to select']);
				else if(sWord === 'GetSelect') return hover('The Window.GetSelect function returns the window ID of the selected window. If the select window is the main (default) run window, then it returns defWinID (which is -1)', 'Window.GetSelect : int', '(Function)', [], [], 'Identifier of currently selected window');
				else if(sWord === 'SetActive') return hover('The Window.SetActive procedure activates the window specified by the windowID parameter', 'Window.SetActive (windowID : int)', '(Procedure)', ['windowID'], ['Identifier of window to set as active']);
				else if(sWord === 'GetActive') return hover('The Window.GetActive function returns the window ID of the active window. If the active window is a Turing run window, then Window.GetActive returns defWinID (which is -1) if the window is the default run window, or whatever number was returned from Window.Open for any other run window', 'Window.GetActive : int', '(Function)', [], [], 'Identifier of currently active window');
				else if(sWord === 'GetPosition') return hover('The Window.GetPosition procedure returns the location of the specified execution window on the screen in the x and y parameters. The x and y parameters specify the lower left corner of the window in screen coordinates. (0, 0) is the lower left corner of the screen', 'Window.GetPosition (windowID : int, var x, y : int)', '(Procedure)', ['windowID', 'x', 'y'], ['Identifier of window', 'Variable set to X position', 'Variable set to Y position']);
				else if(sWord === 'SetPosition') return hover('The Window.SetPosition procedure moves the location of the specified execution window on the screen. x and y specify the lower left corner of the window in screen coordinates. (0, 0) is the lower left corner of the screen', 'Window.SetPosition (windowID : int, x, y : int)', '(Procedure)', ['windowID', 'x', 'y'], ['Identifier of window to move', 'New X position of window', 'New Y position of window']);
				else if(sWord === 'Hide') return hover('The Window.Hide procedure hides the specified window. This means it disappears from the user\'s screen. However, it is still possible to select and draw the window while it remains hidden. If the user activates it (using Window.GetActive) it will automatically appear', 'Window.Hide (windowID : int)', '(Procedure)', ['windowID'], ['Identifier of window to hide']);
				else if(sWord === 'Show') return hover('The Window.Show procedure makes the specified window appear if it was invisible', 'Window.Show (windowID : int)', '(Procedure)', ['windowID'], ['Identifier of window to show']);
				else if(sWord === 'Set') return hover('The Window.Set procedure sets the configuration of the window specified by the windowID parameter. See View.Set for a complete list of available options. The setUpString parameter can be any combination options, separated by commas. Here is a selection of the available options', 'Window.Set (windowID : int, setUpString : string)', '(Procedure)', ['windowID', 'setUpString'], ['Identifier of window to reconfigure', 'Setup string to reconfigure window']);
				else if(sWord === 'Update') return hover('The Window.Update procedure updates a specified Run window from an offscreen bitmap. It is used with the command View.Set("offscreenonly") which prevents the Run window from being updated until the Window.Update command is given', 'Window.Update (windowID : int)', '(Procedure)', ['windowID'], ['Identifier of window to update']);
			}
			if(line.search(moduleRegex) >= 0) {
				if(word === 'Config') return hover('This unit contains the predefined subprograms that deal with getting configuration information about the machine and environment on which the program is being run', 'Dir', '(Module)');
				else if(word == 'Dir') return hover('This unit contains the predefined subprograms that deal with directories. You can use these subprograms to list the contents of directories, create directories, change directories and return the current directory', 'Dir', '(Module)');
				else if(word == 'Draw') return hover('This unit contains the predefined subprograms that deal with drawing pixel graphics to the screen. Most methods require view be set to "graphics"', 'Draw', '(Module)');
				else if(word == 'Error') return hover('This unit contains the predefined subprograms that deal with errors returned from other predefined subprograms', 'Error', '(Module)');
				else if(word == 'File') return hover('This unit contains the predefined subprograms that deal with file manipulation on a whole-file basis (as opposed to manipulating the data in the file using open and close, etc.). These routines allow you to rename, copy and delete files, as well as get information about a file and get the free space on disk available for a file', 'File', '(Module)');
				else if(word == 'Input') return hover('This unit contains the predefined procedures that deal with handling input on a character-by-character basis', 'Input', '(Module)');
				else if(word == 'Limits') return hover('This unit contains constants and functions used in determining the mathematical accuracy of the language', 'Limits', '(Module)');
				else if(word == 'Math') return hover('This unit contains all the mathematical routines. There are three routines that are part of the language, but are conceptually part of the Math unit', 'Math', '(Module)');
				else if(word == 'Mouse') return hover('This unit contains the predefined subprograms that deal with using the mouse in a Turing program', 'Mouse', '(Module)');
				else if(word == 'Music') return hover('This unit contains the predefined subprograms that deal with sound and music. Some of these routines have not been implemented at the time of the writing of this manual and will be implemented in future releases', 'Music', '(Module)');
				else if(word == 'Net') return hover('The Net module allows TCP/IP equipped machines to communicate. In the current implementation (WinOOT 3.0), this is available only under Win32 (Windows 95, 98, NT and later)', 'Net', '(Module)');
				else if(word == 'PC') return hover('This unit contains the predefined subprograms that deal with direct access to the hardware under the IBM PC architecture', 'PC', '(Module)');
				else if(word == 'RGB') return hover('This unit contains the predefined constants for the basic colors and the subprograms to change the color palette', 'RGB', '(Module)');
				else if(word == 'Stream') return hover('This unit contains the predefined subprograms that deal with I/O streams. The basic I/O in Turing is done with I/O statements. However, extra functions are all part of the Stream unit', 'Stream', '(Module)');
				else if(word == 'Str') return hover('This unit contains the predefined constants for manipulating strings', 'Str', '(Module)');
				else if(word == 'Sys') return hover('This unit contains the predefined subprograms that deal with the operating system directly (getting the process id, getting run time arguments and executing commands in the operating system, etc.)', 'Sys', '(Module)');
				else if(word == 'Text') return hover('This unit contains the predefined subprograms that handle character (text) output on the screen (i.e. output using put)', 'Text', '(Module)');
				else if(word == 'Time') return hover('This unit contains the predefined subprograms that handle anything to do with time, either as a date or as a timer', 'Time', '(Module)');
